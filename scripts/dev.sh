...
build)
	...
	# Run RocksDB build optimization
	if [ -f "scripts/optimize_rocksdb_build.sh" ]; then
		./scripts/optimize_rocksdb_build.sh
	fi

	# Check if we should disable default rocksdb features for faster compilation
	if [ "${SKIP_ROCKSDB_DEFAULT_FEATURES:-false}" = "true" ]; then
		warning "Disabling rocksdb default compression features for faster build"
		export RUSTFLAGS="$RUSTFLAGS --cfg disable_rocksdb_defaults"
	fi

	# Check if librocksdb-sys is cached or using system library
	...
	if [ -n "${ROCKSDB_LIB_DIR}" ] && [ -n "${ROCKSDB_INCLUDE_DIR}" ]; then
		success "✓ Using system rocksdb library"
	elif ls $TARGET_DIR/deps/*rocksdb*.rlib 1>/dev/null 2>&1; then
